// 💡 React TaskCard - With Comments, Styled to Match Screenshot
 ...
 const handleAddComment = async () => {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:5000/api/tasks/${task._id}`, {
      ...
      body: JSON.stringify({
        comments: commentText, // ✅ The magic fix
      }),
    });

    const data = await response.json();
    setTask(data.data);       // 🧼 Now with updated comments
    setCommentText("");       // 🧽 Clean that input field
  } catch (error) {
